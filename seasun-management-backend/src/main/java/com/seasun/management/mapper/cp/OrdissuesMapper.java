...
public interface OrdissuesMapper {
    @Delete({
        "delete from cp.ordissues",
        "where ID = #{id,jdbcType=INTEGER}"
    })
    int deleteByPrimaryKey(Integer id);

    @Insert({
        "insert into cp.ordissues (BriefDescription, FullDescription, ",
        "CreatedBy, ClosedBy, ",
    ...
    int insert(Ordissues record);

    int insertSelective(Ordissues record);
    ...
    @ResultMap("com.seasun.management.mapper.cp.OrdissuesMapper.BaseResultMap")
    Ordissues selectByPrimaryKey(Integer id);

    int updateByPrimaryKeySelective(Ordissues record);
    ...
    int updateByPrimaryKey(Ordissues record);

    /* the flowing are user defined ... */

    @Select({"SELECT date_format(min(CreatedOn),'%Y') startYear,date_format(max(CreatedOn),'%Y') endYear FROM cp.ordissues where status!=15"})
    BasePeriodAppVo selectPeriod();

    BasePeriodAppVo selectPeriodByCPProjectIds(List<Integer> cPProjectIds);
}